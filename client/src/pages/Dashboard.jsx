...
import { useGetDasboardStatsQuery, useGetAllTaskQuery } from "../redux/slices/api/taskApiSlice";
import { BGS, PRIOTITYSTYELS, TASK_TYPE, getInitials } from "../utils";
import { useSelector } from "react-redux";

const Card = ({ label, count, bg, icon }) => {
  return (
    <div className='w-full h-28 sm:h-32 lg:h-36 bg-white p-3 sm:p-4 md:p-5 shadow-md rounded-md flex items-center justify-between'>
      <div className='h-full flex flex-1 flex-col justify-between'>
        <p className='text-sm sm:text-base text-gray-600'>{label}</p>
        <span className='text-lg sm:text-2xl font-semibold'>{count}</span>
        <span className='text-xs sm:text-sm text-gray-400'>{"111 last month"}</span>
      </div>
      <div
        className={clsx(
          "w-8 sm:w-10 h-8 sm:h-10 rounded-full flex items-center justify-center text-white",
          bg
        )}
      >
        {icon}
...
const Dashboard = () => {
  ...
  // Fetch less than 3 days tasks for analytics
  ...
  const lessThan3DaysCount = lessThan3DaysData?.tasks?.length || 0;
  ...
  const stats = [
    ...
    },
    {
      _id: "5",
      label: "< 3 DAYS",
      total: loadingLessThan3Days ? "..." : lessThan3DaysCount,
      icon: <MdAccessTime />,
      bg: "bg-emerald-500",
    },
  ];
